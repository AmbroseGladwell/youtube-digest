...
test("an empty library shows the plain generate form, not the filter/library chrome", async ({ launcher }) => {
  const form = await launcher.launchExpectingEmptyStateForm();
  await form.apiKeysPanel.verifyIsShown();
});
