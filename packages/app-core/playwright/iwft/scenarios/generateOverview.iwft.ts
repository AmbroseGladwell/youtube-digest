...
test("a successful generation transitions the home page from the empty form to the library, with the new card expanded", async ({
  launcher,
}) => {
  const home = await launcher.launch({ apiKeys: API_KEYS });
  const form = await home.verifyShowsEmptyStateForm();
...
test("the transcript-fetch phase surfaces its error, and generation never reaches Anthropic", async ({
  ...
  const form = await launcher.launchExpectingEmptyStateForm({ apiKeys: API_KEYS });
